...
function toRestaurant(item: any): ParsedRestaurant | null {
  try {
    ...
    const address = addressObj?.streetAddress
      ...
      : firstString(addressObj ?? 'Buea, Cameroon');
    ...
    console.log('[buea.parse] error converting item', e);
    return null;
  }
}
  ...
  while ((match = scriptRegex.exec(html)) !== null) {
    ...
    try {
      ...
      try {
        ...
        console.log('[buea.jsonld] parse failure, skipping block');
      }
    }
  }
  return results;
...
export const fetchBueaRestaurantsProcedure = publicProcedure
  .input(
    z
      .object({
  ...
  .query(async ({ input }) => {
    ...
    console.log('[tRPC] Fetch Buea restaurants', { targetUrls });
