...
function toRestaurant(item: any): ParsedRestaurant | null {
  try {
    ...
    const address = addressObj?.streetAddress
      ...
      : firstString(addressObj ?? 'Douala, Cameroon');
    ...
    const restaurant: ParsedRestaurant = {
      ...
      address: address || 'Douala, Cameroon',
      priceRange,
      isOpen: true,
      tags,
    };
    ...
    console.log('[douala.parse] error converting item', e);
    return null;
  }
}
...
function extractJsonLd(html: string): any[] {
  ...
  while ((match = scriptRegex.exec(html)) !== null) {
    ...
    try {
      ...
      try {
        // Some pages embed multiple JSON objects concatenated; try to wrap in array
        const wrapped = `[${raw.replace(/}\s*{/, '},{')}]`;
        const json = JSON.parse(wrapped);
        results.push(...(Array.isArray(json) ? json : [json]));
      } catch (e2) {
        console.log('[douala.jsonld] parse failure, skipping block');
      }
    }
  }
  return results;
...
export const fetchDoualaRestaurantsProcedure = publicProcedure
  ...
  .query(async ({ input }) => {
    const baseUrl = 'https://www.tripadvisor.com/Restaurants-g297392-Douala_Littoral_Region.html';

    function pageToUrl(p: number): string {
      const offset = (p - 1) * 30;
      if (offset <= 0) return baseUrl;
      return `https://www.tripadvisor.com/Restaurants-g297392-oa${offset}-Douala_Littoral_Region.html`;
    }
    ...
    console.log('[tRPC] Fetch Douala restaurants', { targetUrls });
